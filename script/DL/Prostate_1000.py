...
def get_top_1000_features(k=1000):

    file = 'WithClinical/0_train_X4.mat'
    A = loadmat(file)
    R = A['R'][0]
    Y = A['Y'][0].astype('int32')
    X = A['X'].astype('float32')
    ...
    Fea = Feature[idx]
    with open('Prostate.txt', 'w') as fp:
        for fea in Fea:
            fp.write("%s\n" % fea)
        print('Done')
