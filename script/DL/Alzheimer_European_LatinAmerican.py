...
def get_fam(sub_list):
    df_fam = pd.read_csv('Hispanic/EA_Hispanic.fam', sep='\t', header=None)
    df_fam.columns = ['FID', 'IID', 'FAT', 'MAT', 'SEX', 'PHE']
    df = df_fam[df_fam['IID'].isin(sub_list)]
    df = df[['FID', 'IID']]
    return df

def get_gwas_data(file, seed=0):
    df = pd.read_csv(file, sep=',', index_col=0)
    df['YR'] = df['Y'].map(str) + df['R']
    df = df[df['Y'].isin([0,1])]
    ...
    train, test = train_test_split(df, test_size=0.2, random_state=0, shuffle=True, stratify=df['YR'])
    df_train_mix0 = train
    df_train_ind1 = train[train['R'] == 'EA']
    df_train_ind2 = train[train['R'] == 'Hispanic']

    df_train_mix0 = get_fam(df_train_mix0.index)
    df_train_ind1 = get_fam(df_train_ind1.index)
    df_train_ind2 = get_fam(df_train_ind2.index)

    df_train_mix0.to_csv('Hispanic/gwas/train/mix_train.txt', index=False, sep='\t', header=None)
    df_train_ind1.to_csv('Hispanic/gwas/train/ind1_train.txt', index=False, sep='\t', header=None)
    df_train_ind2.to_csv('Hispanic/gwas/train/ind2_train.txt', index=False, sep='\t', header=None)
    print(df_train_mix0.shape, df_train_ind1.shape, df_train_ind2.shape)

    for seed in range(20):
        val_samples, test_samples = train_test_split(test, test_size=0.5, random_state=seed, shuffle=True, stratify=test['YR'])
        df_test_mix = test_samples
        df_test_1 = test_samples[test_samples['R'] == 'EA']
        df_test_2 = test_samples[test_samples['R'] == 'Hispanic']
        # df_test_mix.to_csv('Hispanic/gwas/test/mix{}.csv'.format(seed), index=True, sep='\t', header=True)

        if seed == 0:
            df_test_mix = get_fam(df_test_mix.index)
            df_test_1 = get_fam(df_test_1.index)
            df_test_2 = get_fam(df_test_2.index)

            df_test_mix.to_csv('Hispanic/gwas/test/mix_test.txt', index=False, sep='\t', header=None)
            df_test_1.to_csv('Hispanic/gwas/test/ind1_test.txt', index=False, sep='\t', header=None)
            df_test_2.to_csv('Hispanic/gwas/test/ind2_test.txt', index=False, sep='\t', header=None)
            print(df_test_mix.shape, df_test_1.shape, df_test_2.shape)
...
def run_cv(seed=0):
    ...
    Aggr, EA, Minor = get_data("Hispanic/EA_Hispanic1.csv", seed=seed)
