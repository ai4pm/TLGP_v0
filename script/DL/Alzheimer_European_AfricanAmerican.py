...
def get_fam(sub_list):
    df_fam = pd.read_csv('AA/EA_AA.fam', sep='\t', header=None)
...
def run_cv(seed):
    ...
    Aggr, EA, Minor = get_data("AA/EA_AA1.csv", seed=seed)
    # return ['Mix0', 'Mix1', 'Mix2', 'Ind1', 'Ind2', 'naive', 'TL2']
    # Aggr, EA, Minor = get_data("AA/EA_AA2.csv", seed=seed)
...
def main():
    arguments = sys.argv
    print(arguments)
    # get_gwas_data("AA/EA_AA1.csv", seed=0)
    res = []
    for seed in range(1):
        row = run_cv(seed)
        res.append(row)
